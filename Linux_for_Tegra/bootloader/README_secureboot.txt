...
Example fuses handled by L4T secureboot are as follows:
    ...
    |       |                     |                                   |
    |      1| pkc_disable         | PKC - 0x1, NS - 0x1 (T210)        |
    |       |                     |                                   |
    |    128| secure_boot_key     | Secure Boot Key (SBK)             |
    |       |                     | AES encryption key for encrypting |
    |       |                     | bootloader (for T186 and T194).   |
    |     32| device_key          | Device Key (DK for T210)          |
    |       |                     | Device key for other security     |
    |       |                     | applications. If no other         |
    |       |                     | security applications are         |
    |       |                     | used, leave it untouched.         |
    |    128| Key Encryption Key 0| KEK0 (T186 and T194)              |
    |    128| Key Encryption Key 1| KEK1 (T186 and T194)              |
    |    256| KEK256 = KEK0 + KEK1| KEK256 (T186 and T194)            |
    |    128| Key Encryption Key 2| KEK2 (T186 and T194)              |
...
Applies to: Jetson TX2 and Jetson AGX Xavier devices
...
- To protect odm production fuse with JTAG disabled (for T210):
  ...
  -l 0xX        Sets odm_lock=0xX. Setting each bit locks the corresponding
                32-bits odm_reserved.
                For example, setting odmlock=0x1 locks the first 32-bit of the
                odm_reserved read only.
                Setting odmlock=0x5 locks the first and third 32-bits of the
                odm_reserved field read only and so on.

  -o <value>    Sets odm_reserved=<value>. The value must be a quoted series of
                8 32-bit HEX numbers such as:
"0xXXXXXXXX 0xXXXXXXXX 0xXXXXXXXX 0xXXXXXXXX 0xXXXXXXXX 0xXXXXXXXX 0xXXXXXXXX 0x00000000"
                ...
                fuses are reserved for NVIDIA use.
...
========================================================================
  ...
  ======================================================================
  Use both PKC and SBK keys for encryption and signing (for TX2 and Xavier)
  ======================================================================
  1. Navigate to the directory where you installed L4T.
  2. Place the Tegra device into force recovery mode.
  3. Run command below:
...
========================================================================
 ...
 =======================================================================
  ...
  ======================================================================
  Use both PKC and SBK keys (For TX2 and Xavier)
  ======================================================================
  1. Navigate to the directory where you installed L4T.
...
burning fuses in a factory environment.
  ...
  -----------------------------------------------------
  Building the fuse blob in a factory environment requires:
   - Set up the preparation of the key host in safe location
     See Installing the L4T Secureboot Package for details.
   - Generate the RSA Key-pair
     See Generating the RSA Key Pair for details.
   - If necessary, prepare the DK(KEK)/SBK/ODM fuses
     See Preparing the DK(KEK)/SBK/ODM Fuses for details.
   ...
   - For Jetson Xavier:
     ...
     2. Execute the command.
       $ sudo ./odmfuse.sh --noburn -i 0x19 -c PKC -p -k <key.pem> \
       [--KEK0 <KEK0 file> --KEK1 <KEK1 file> --KEK2 <KEK2 file> \
       --KEK256 <KEK256 file> -S <SBK file>] jetson-xavier

       This standard Jetson Xavier fuse command requires an actual
       target device connected to the host and placed in
       force recovery mode.

    To generate the fuse blob without an actual Jetson target
    device attached to the host:
    - Add "BOARDID=2888 FAB=400 BOARDSKU=0001 BOARDREV=H.0 CHIPREV=2"
      in front of "./odmfuse.sh" as follows:
      sudo BOARDID=2888 FAB=400 BOARDSKU=0001 BOARDREV=H.0 CHIPREV=2 \
      $ sudo ./odmfuse.sh --noburn -i 0x19 -c PKC -p -k <key.pem> \
      [--KEK0 <KEK0 file> --KEK1 <KEK1 file> --KEK2 <KEK2 file> \
      --KEK256 <KEK256 file> -S <SBK file>] jetson-xavier
   ...
   - For Jetson TX2:
     1. Navigate to the directory where you installed L4T.
     2. Execute the command.
       $ sudo ./odmfuse.sh --noburn -i 0x18 -c PKC -p -k <key.pem> \
       [--KEK0 <KEK0 file> --KEK1 <KEK1 file> --KEK2 <KEK2 file> \
       --KEK256 <KEK256 file> -S <SBK file>] jetson-tx2

       This standard Jetson TX2 fuse command requires an actual
       target device connected to the host and placed in
       force recovery mode.

    To generate the fuse blob without an actual Jetson target
    device attached to the host:
    - Add "BOARDID=3310 FAB=C04" in front of "./odmfuse.sh" as follows:
      $ sudo BOARDID=3310 FAB=C04 ./odmfuse.sh --noburn -i 0x18 -c PKC -p \
	  -k <key.pem> [--KEK0 <KEK0 file> --KEK1 <KEK1 file> \
	  --KEK2 <KEK2 file> --KEK256 <KEK256 file> -S <SBK file>] jetson-tx2

  - For Jetson TX1:
    1. Navigate to the directory where you installed L4T.
    2. Execute the command.
       $ sudo ./odmfuse.sh --noburn -i <chipid> -c PKC -p -k <key.pem> \
       [-D <DK file> -S <SBK file>] jetson-tx1

  - For Jetson Nano Production Module:
    1. Navigate to the directory where you installed L4T.
    2. Execute the command.
       $ sudo ./odmfuse.sh --noburn -i <chipid> -c PKC -p -k <key.pem> \
       [-D <DK file> -S <SBK file>] jetson-nano-emmc

  Burning the Fuse Blob in a Factory Environment
  ----------------------------------------------
  Burning the fuse blob in a factory environment requires:
  - Set up a X86 Linux host as the factory fuse host.
    The factory fuse host does not require an L4T BSP installation.
  - Download fuseblob.tbz2 to factory fuse host with the command.
  ...
  2. Execute the command:
     ...
     NOTE: This procedure can be used and repeated on multiple
           Tegra boards. All the boards are burned with same
           fuseblob and have exactly same fuse configurations.

========================================================================
Example: Flashing Signed Images in a Factory Environment
========================================================================
This topic provides an example reference implementation for
flashing signed images in a factory environment. Building
signed images in a trusted environment requires:

- Set up the signing host in a safe location.
  See Installing the L4T Secureboot Package for details.
- Use the following procedure to prepare the signed image blob.

  To prepare the signed image blob:
  1. Navigate to the directory where you installed L4T.
     $ cd <Your Local L4T Path>.../Linux_for_Tegra

  2. If you are using a different security application, prepare the files
     using the user guide for that application.

  3. Clean up the stale and unwanted content inside flashcmd.txt:
     $ sudo rm -f bootloader/flashcmd.txt

  4. Generate the signed binary image:
     - For Jetson Xavier:
       $ sudo BOARDID=2888 FAB=400 BOARDSKU=0001 BOARDREV=H.0 ./flash.sh \
         --no-flash -u <pkc_keyfile> [-v <sbk_keyfile>] \
         jetson-xavier mmcblk0p1
     ...
     - For Jetson TX1:
       $ sudo ./flash.sh --no-flash -x 0x21 -y PKC -u <pkc_keyfile> \
         jetson-tx1 mmcblk0p1

     - For Jetson Nano Production Module:
       $ sudo ./flash.sh --no-flash -x 0x21 -y PKC -u <pkc_keyfile> \
         jetson-nano-emmc mmcblk0p1

  5. Save signed binaries to signedimage.tbz2:
      $ sudo rm bootloader/*.raw
      $ sudo tar cvjf signedimage.tbz2 bootloader

  6. Flash the signed images in factory environment:

     Requirements:
     - Set up a X86 Linux host as the "factory flash host".
       The factory flash host does not require the L4T BSP installation.
     - Download signedimage.tbz2 to the factory flash host with the command.
       $ sudo tar xvjf <download path>.../signedimage.tbz2
     - Use the following procedure to flash signed images.

     To flash the signed images:
     1. Put the Tegra device into force recovery mode.
     2. Execute the command:
        $ cd bootloader
        $ sudo bash ./flashcmd.txt
     ...
     NOTE: This procedure can be used and repeated on multiple
           properly fused Tegra boards. All Tegra boards are
           burned with the same fuseblob and have exactly the
           same signed images flashed.
